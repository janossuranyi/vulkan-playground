...
namespace jsr {
    std::vector<VkClearValue> CompImageGen::getClearValues() const
    {
        return std::vector<VkClearValue>();
    }

    VkResult CompImageGen::createPipeline(const ComputePipelineShaderBinary& shaders)
    {
        ...
        /* pipeline */
        const VkDescriptorSetLayout layouts[] = { m_engine->getGlobalVarsLayout(), m_engine->getGlobalTexturesLayout(), m_passLayout };
