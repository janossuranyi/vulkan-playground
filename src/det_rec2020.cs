...
layout(set = 0, binding = 2) uniform sampler _562;
layout(set = 0, binding = 3) uniform texture2D _726;
layout(set = 0, binding = 4) uniform sampler _728;
...
layout(set = 0, binding = 10) uniform sampler _1665;
layout(set = 1, binding = 10) uniform texture3D _1707;
...
layout(set = 0, binding = 12) uniform sampler _1901;
...
float _98(vec2 _97)
{
    return (textureLod(sampler2D(_558, _562), vec4(_97, 0.0, 0.0).xy, vec4(_97, 0.0, 0.0).w).x - 0.5) * 2.0;
}
...
void main()
{
    ...
    vec2 _724 = (textureLod(sampler2D(_726, _728), vec4(_609, 0.0, 0.0).xy, vec4(_609, 0.0, 0.0).w).xy - vec2(0.4980392158031463623046875)) * 0.0199999995529651641845703125;
    _724.x *= _627;
    float _752 = textureLod(sampler2D(_726, _728), vec4(_609 + _724, 0.0, 0.0).xy, vec4(_609 + _724, 0.0, 0.0).w).z;
    ...
    if (_870 || (_866 != 0.0))
    {
        ...
        if (_866 != 0.0)
        {
            ...
            if (_870)
            {
                _914 = textureLod(sampler2D(_918, _728), vec4(_901, _772.y, 0.0, 0.0).xy, vec4(_901, _772.y, 0.0, 0.0).w).w;
            }
            _934 = vec3(0.0);
            SPIRV_CROSS_BRANCH
            if (_914 != 0.0)
            {
                vec3 _940 = textureLod(sampler2D(_941, _728), vec4(_901 + _910, _772.y, 0.0, 0.0).xy, vec4(_901 + _910, _772.y, 0.0, 0.0).w).xyz;
                vec3 _961 = textureLod(sampler2D(_941, _728), vec4(_901 - _910, _772.y, 0.0, 0.0).xy, vec4(_901 - _910, _772.y, 0.0, 0.0).w).xyz;
                _934 += ((_940 + _961) * (_914 * 0.5));
            }
            SPIRV_CROSS_BRANCH
            if (_914 != 1.0)
            {
                vec3 _993 = textureLod(sampler2D(_994, _728), vec4(_901 + _910, _772.y, 0.0, 0.0).xy, vec4(_901 + _910, _772.y, 0.0, 0.0).w).xyz;
                vec3 _1014 = textureLod(sampler2D(_994, _728), vec4(_901 - _910, _772.y, 0.0, 0.0).xy, vec4(_901 - _910, _772.y, 0.0, 0.0).w).xyz;
        ...
        else
        {
            ...
            if (_870)
            {
                _1044 = textureLod(sampler2D(_918, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).w;
            }
            _934 = vec3(0.0);
            SPIRV_CROSS_BRANCH
            if (_1044 != 0.0)
            {
                _934 += (textureLod(sampler2D(_941, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).xyz * _1044);
            }
            SPIRV_CROSS_BRANCH
            if (_1044 != 1.0)
            {
                _934 += (textureLod(sampler2D(_994, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).xyz * (1.0 - _1044));
            }
        }
    }
    else
    {
        _934 = textureLod(sampler2D(_994, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).xyz;
    }
    ...
    if (_1135)
    {
        ...
        if (_1138 != 0.0)
        {
            ...
            _934.x = textureLod(sampler2D(_994, _728), vec4(_772 - _1152, 0.0, 0.0).xy, vec4(_772 - _1152, 0.0, 0.0).w).x;
            _934.y = textureLod(sampler2D(_994, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).y;
            _934.z = textureLod(sampler2D(_994, _728), vec4(_772 + _1152, 0.0, 0.0).xy, vec4(_772 + _1152, 0.0, 0.0).w).z;
        }
    }
    ...
    if ((_1212 & 16u) != 0u)
    {
        vec4 _1224 = textureLod(sampler2D(_1225, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w);
    ...
    if (_617._m2.x > 0.0)
    {
        vec4 _1259 = textureLod(sampler2D(_1260, _728), vec4(_609, 0.0, 0.0).xy, vec4(_609, 0.0, 0.0).w);
        float _1278 = _1259.w;
    ...
    if (_281._m47 == 5u)
    {
        vec4 _1289 = textureLod(sampler2D(_1290, _728), vec4(_609, 0.0, 0.0).xy, vec4(_609, 0.0, 0.0).w);
        ...
        vec3 _1452 = vec3(0.0);
        _1452.x = textureLod(sampler2D(_994, _728), vec4(_1424, 0.0, 0.0).xy, vec4(_1424, 0.0, 0.0).w).x;
        _1452.y = textureLod(sampler2D(_994, _728), vec4(_1434, 0.0, 0.0).xy, vec4(_1434, 0.0, 0.0).w).y;
        _1452.z = textureLod(sampler2D(_994, _728), vec4(_1442, 0.0, 0.0).xy, vec4(_1442, 0.0, 0.0).w).z;
        float _1503 = _1315;
        _934 = mix(_934, _1452, vec3(_10(_1503)));
    }
    vec4 _1508 = _617._m6;
    float _1512 = textureLod(sampler2D(_1513, _728), vec2(0.5), 0.0).x;
    float _1519 = _1508.w;
    _1512 = (_1519 != 0.0) ? _1519 : _1512;
    vec3 _1527 = textureLod(sampler2D(_1528, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).xyz;
    vec3 _1544 = textureLod(sampler2D(_1545, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).xyz;
    vec3 _1561 = textureLod(sampler2D(_1562, _562), vec4((_609 * 1.39999997615814208984375) * vec2(1.0, _627), 0.0, 0.0).xy, vec4((_609 * 1.39999997615814208984375) * vec2(1.0, _627), 0.0, 0.0).w).xyz;
    SPIRV_CROSS_BRANCH
    if (_617._m15 > 0.0)
    {
    ...
    if (_281._m51 > 0.0)
    {
        _934 = textureLod(sampler2D(_1641, _728), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).xyz;
    }
    SPIRV_CROSS_BRANCH
    if (_617._m16 > 0.0)
    {
        ...
        _934 = textureLod(sampler3D(_1707, _728), vec4(_1692, 0.0).xyz, vec4(_1692, 0.0).w).xyz;
    }
    ...
    if ((_1745 & 7u) != 0u)
    {
        ...
        if (_1825 && (_1782 == 0u))
        {
            vec4 _1836 = textureLod(sampler2D(_1837, _728), vec4(_609, 0.0, 0.0).xy, vec4(_609, 0.0, 0.0).w);
    ...
    if (_281._m6.w > 0.0)
    {
        float _1898 = 1.0 - textureLod(sampler2D(_1899, _1901), vec4(_772, 0.0, 0.0).xy, vec4(_772, 0.0, 0.0).w).x;
        float _1920 = _1898;
        bool _1922 = false;
    ...
    if (_617._m9.w != 0.0)
    {
        ...
        vec2 _2159 = textureLod(sampler2D(_2160, _728), vec4(_609, 0.0, 0.0).xy, vec4(_609, 0.0, 0.0).w).xy;
        _2159 = (_2159 * 2.0) - vec2(1.0);
        float _2180 = textureLod(sampler2D(_2160, _728), vec4(_609 + (_2159 * _2145), 0.0, 0.0).xy, vec4(_609 + (_2159 * _2145), 0.0, 0.0).w).z;
        float _2204 = textureLod(sampler2D(_2160, _728), vec4(_609 + (_2159 * _2149), 0.0, 0.0).xy, vec4(_609 + (_2159 * _2149), 0.0, 0.0).w).z;
        float _2230 = _2154;
    ...
    if (_2517.w != 0.0)
    {
        ...
        if (_2534 != 0.0)
        {
            ...
            vec3 _2562 = textureLod(sampler2D(_2563, _728), vec4(_609, 0.0, 0.0).xy, vec4(_609, 0.0, 0.0).w).xyw;
            vec2 _2579 = ((_2562.xy * 2.0) - vec2(1.0)) * (_2534 * _2562.z);
            _2579.x *= _627;
            vec2 _2595 = _609 - (_2579 * (_2544 - 0.5));
            vec2 _2602 = _609 - (_2579 * (_2550 - 0.5));
            _2609 = mix(textureLod(sampler2D(_2610, _2612), vec4(_2595 * _2537, 0.0, 0.0).xy, vec4(_2595 * _2537, 0.0, 0.0).w), textureLod(sampler2D(_2610, _2612), vec4(_2602 * _2537, 0.0, 0.0).xy, vec4(_2602 * _2537, 0.0, 0.0).w), vec4(_2557));
        }
        else
        {
            _2609 = textureLod(sampler2D(_2610, _2612), vec4(_609 * _2537, 0.0, 0.0).xy, vec4(_609 * _2537, 0.0, 0.0).w);
        }
    ...
    if (_2517.w != 0.0)
    {
        ...
        if (_2771 != 0.0)
        {
            ...
            vec3 _2799 = textureLod(sampler2D(_2800, _728), vec4(_609, 0.0, 0.0).xy, vec4(_609, 0.0, 0.0).w).xyw;
            vec2 _2816 = ((_2799.xy * 2.0) - vec2(1.0)) * (_2771 * _2799.z);
            _2816.x *= _627;
            vec2 _2832 = _609 - (_2816 * (_2781 - 0.5));
            vec2 _2839 = _609 - (_2816 * (_2787 - 0.5));
            _2846 = mix(textureLod(sampler2D(_2847, _2849), vec4(_2832 * _2774, 0.0, 0.0).xy, vec4(_2832 * _2774, 0.0, 0.0).w), textureLod(sampler2D(_2847, _2849), vec4(_2839 * _2774, 0.0, 0.0).xy, vec4(_2839 * _2774, 0.0, 0.0).w), vec4(_2794));
        }
        else
        {
            _2846 = textureLod(sampler2D(_2847, _2849), vec4(_609 * _2774, 0.0, 0.0).xy, vec4(_609 * _2774, 0.0, 0.0).w);
        }
    ...
    if (_2517.w != 0.0)
    {
        ...
        if (_3007 != 0.0)
        {
            ...
            vec3 _3035 = textureLod(sampler2D(_3036, _728), vec4(_609, 0.0, 0.0).xy, vec4(_609, 0.0, 0.0).w).xyw;
            vec2 _3052 = ((_3035.xy * 2.0) - vec2(1.0)) * (_3007 * _3035.z);
            _3052.x *= _627;
            vec2 _3068 = _609 - (_3052 * (_3017 - 0.5));
            vec2 _3075 = _609 - (_3052 * (_3023 - 0.5));
            _3082 = mix(textureLod(sampler2D(_3083, _3085), vec4(_3068 * _3010, 0.0, 0.0).xy, vec4(_3068 * _3010, 0.0, 0.0).w), textureLod(sampler2D(_3083, _3085), vec4(_3075 * _3010, 0.0, 0.0).xy, vec4(_3075 * _3010, 0.0, 0.0).w), vec4(_3030));
        }
        else
        {
            _3082 = textureLod(sampler2D(_3083, _3085), vec4(_609 * _3010, 0.0, 0.0).xy, vec4(_609 * _3010, 0.0, 0.0).w);
        }
    ...
    if (_281._m16.x > 0.0)
    {
        vec4 _3292 = textureLod(sampler2D(_3293, _728), vec4((_609 * vec2(1.2999999523162841796875)) + (floor(vec2(_281._m48 * 15.0, _281._m48 * 20.0)) * 0.300000011920928955078125), 0.0, 0.0).xy, vec4((_609 * vec2(1.2999999523162841796875)) + (floor(vec2(_281._m48 * 15.0, _281._m48 * 20.0)) * 0.300000011920928955078125), 0.0, 0.0).w);
    ...
    if (_281._m47 == 5u)
    {
        ...
        float _3517 = textureLod(sampler2D(_3499, _562), vec4(_3477 * 4.30000019073486328125, 0.0, 0.0).xy, vec4(_3477 * 4.30000019073486328125, 0.0, 0.0).w).x;
        float _3498 = _10(_3517);
        float _3540 = textureLod(sampler2D(_3499, _562), vec4(_3477 * 0.07299999892711639404296875, 0.0, 0.0).xy, vec4(_3477 * 0.07299999892711639404296875, 0.0, 0.0).w).x + 0.64999997615814208984375;
        _3498 *= _10(_3540);
    ...
    else
    {
        ...
        vec3 _3750 = _934;
        _934 = max(vec3(0.0), mix(vec3(_52(_3750)), _934, vec3(_281._m53)));
        vec3 _3762 = _934;
        float _3764 = _281._m52;
        vec3 _3761 = _85(_3762, _3764);
        _3658 = textureLod(sampler3D(_3768, _728), vec4(_3761, 0.0).xyz, vec4(_3761, 0.0).w).xyz;
    }
    SPIRV_CROSS_FLATTEN
    if (_281._m47 == 3u)
    {
